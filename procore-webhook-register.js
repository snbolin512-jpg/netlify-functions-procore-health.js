...
exports.handler = async function (event) {
  ...
  try {
    ...
    let hook = null;
    const existing = await api(`/rest/v1.0/webhooks/hooks?company_id=${companyId}`);
    ...
    let current = [];
    try {
      current = await api(`/rest/v1.0/webhooks/hooks/${hook.id}/triggers?company_id=${companyId}`);
    ...
    for (const t of TRIGGERS) {
      ...
      try {
        await api(`/rest/v1.0/webhooks/hooks/${hook.id}/triggers`, {
          method: "POST",
          body: { company_id: companyId, api_version: "v2", trigger: t }
        });
        added.push(key);
      } catch (e) {
