...
### Getting to your real project

Everything above is the **Development Sandbox** — a separate Procore instance
with invented seed data. Your real job is not in it and never will be. There is
no way to copy a production project into a Development Sandbox.

To point OhmBoy at the real project you move the app to production. You do
**not** need to publish to the Procore Marketplace or go through partner
verification for this — that is only required to distribute your app to *other*
companies. Installing your own custom app in your own company is supported
directly.

What you do need is **Company Admin permission** in the real Procore account.
If you don't have it, find whoever does; steps 3 and 4 below cannot be done
without it.

1. **Promote the version.** In the Configuration Builder, click
   **Promote Version**.

2. **Save the production secret immediately.** Procore shows the production
   Client ID and Client Secret in a popup. The secret is visible exactly once
   and cannot be retrieved afterwards — only reset. Copy both now.

3. **Add the production Redirect URI.** Under **Production OAuth Credentials**
   on the Manage App page, add the same callback URL. Production and sandbox
   keep separate redirect URI lists; setting one does not set the other.

4. **Install the app in the real company.** Copy the **Production App Version
   Key**, then in your live Procore: **Company Tools → Admin → App Management
   → Install App → Install Custom App**, paste the key, **Install**, confirm.

5. **Repoint OhmBoy.** In Netlify, change:

   | Variable | New value |
   |---|---|
   | `PROCORE_CLIENT_ID` | production Client ID |
   | `PROCORE_CLIENT_SECRET` | production Client Secret |
   | `PROCORE_OAUTH_BASE` | `https://login.procore.com` |
   | `PROCORE_API_BASE` | `https://api.procore.com` |
   | `PROCORE_COMPANY_ID` | your real company id |

   Redeploy, then re-authorise from the Procore Link page. The panel will stop
   saying "sandbox" once you're on production hosts.

6. **Check project access.** With User Level Authentication, OhmBoy sees
   exactly what the person who authorised can see. If the project doesn't show
   in the dropdown, that Procore user isn't on the project — add them to the
   project directory with at least read access on Drawings and Schedule.

No OhmBoy code changes. The sandbox and production difference is entirely
