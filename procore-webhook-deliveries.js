...
exports.handler = async function (event) {
  ...
  try {
    ...
    // Delivery scope must match the hook's scope.
    const scopeQuery = q.project_id ? `project_id=${Number(q.project_id)}` : `company_id=${c.companyId}`;
    const rows = await api(
      `/rest/v1.0/webhooks/hooks/${hookId}/deliveries?${scopeQuery}&filter[status]=${status}`
    );
    return json(200, {
      ok: true, hookId, status,
      count: Array.isArray(rows) ? rows.length : null,
      deliveries: rows,
      readingTheLog: "Check response_status, response_error and outcome (ok / discarded / retried / failed) on each row."
    });
  } catch (err) {
