...
## V0.21.1 Luxe Dashboard Fix

This revision corrects the first luxe pass.

Fixes:
- Smoother, less clunky cockpit composition.
- Floating Cockpit button fixed to top-left instead of stretching across the frame.
- Larger clean Jackson asset with transparent background; no visible background watermark.
- PM Daily Update hero balanced with more top breathing room and no title cropping.
- Financial Source Map connector lines constrained to the map section.
- Command Module icons replaced with crisp inline SVG icons.
- Cockpit summary cards now watch state changes from drawing/schedule REV sims and update.
- Financial map values are higher contrast.
- Color palette moves away from peach/gold/brown into ivory + true copper + black with restrained teal/red.


V0.21.2 stability patch: removed unstable duplicated cockpit patch scripts, removed legacy top cockpit bar element, and installed a single stable cockpit renderer.
